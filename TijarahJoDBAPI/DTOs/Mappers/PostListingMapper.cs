...
public static class PostListingMapper
{
    ...
    public static PostListingResponse ToListingResponse(this DataRow row)
    {
        return new PostListingResponse
        {
            ...
            // Image Info - PrimaryImageUrl from the stored procedure/view
            PrimaryImageUrl = row.Table.Columns.Contains("PrimaryImageUrl") && row["PrimaryImageUrl"] != DBNull.Value
                ? row["PrimaryImageUrl"] as string
                : null
        };
    }
